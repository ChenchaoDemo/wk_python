...
- 提供 Tkinter 可视化界面：输入账号密码并选择执行类型，可刷课或查询考试/作业
...
1. 输入学习通账号和密码；
2. 在账号密码后选择类型：**刷课（选择课程）**、**查询考试列表（待考试）**、**查询作业列表（待做）** 或 **查询全部作业（按课程展示）**；
3. 点击 **新增/登录并执行**；
4. 查询类型会直接在结果表中展示；刷课类型会显示课程列表；
5. 刷课时选中一门或多门课程，再点击 **加入选中课程到刷课列表**。
